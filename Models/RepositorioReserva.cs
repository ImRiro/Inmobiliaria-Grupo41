...
public class RepositorioReserva : RepositorioBase, IRepositorioReserva
{
    ...
    public async Task<List<ReservaPorVencer>> ObtenerQueTerminanEnAsync(int dias)
    {
        var lista = new List<ReservaPorVencer>();
        ...
        var query = @"
            ...
            INNER JOIN Inmueble i ON r.IdInmueble = i.Id
            INNER JOIN Inquilinos q ON r.IdInquilino = q.Id
            WHERE r.Fecha_Hasta >= @Hoy
            AND r.Fecha_Hasta <= @FechaLimite
            AND r.Fecha_Cancelacion IS NULL
            ORDER BY r.Fecha_Hasta";
        ...
        while (await reader.ReadAsync())
        {
            lista.Add(new ReservaPorVencer
            {
                IdReserva = reader.GetInt32(reader.GetOrdinal("Id")),
                DireccionInmueble = reader.GetString(
                    reader.GetOrdinal("DireccionInmueble")
                ),
                NombreInquilino = reader.GetString(
                    reader.GetOrdinal("NombreInquilino")
                ),
                Fecha_Desde = reader.GetDateTime(
                    reader.GetOrdinal("Fecha_Desde")
                ),
                Fecha_Hasta = reader.GetDateTime(
                    reader.GetOrdinal("Fecha_Hasta")
                ),
                Monto_Diario = reader.GetDecimal(
                    reader.GetOrdinal("Monto_Diario")
                ),
                Costo_Total = reader.GetDecimal(
                    reader.GetOrdinal("Costo_Total")
                )
            });
        }
